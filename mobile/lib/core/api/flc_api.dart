...
class FlcApi {
  ...
  Future<List<ListeningAssessmentSummary>> listAssessments(int mediaId) async {
    final json = await _client.get<Map<String, dynamic>>(
      '/listening/media/$mediaId/assessments',
      parser: (d) => d as Map<String, dynamic>,
    );
    return (json['data'] as List<dynamic>)
        .map((e) => ListeningAssessmentSummary.fromJson(e as Map<String, dynamic>))
        .toList();
  }

  String audioUrl(int mediaId) => '$apiBaseUrl/listening/media/$mediaId/audio';

  Future<Map<String, dynamic>> getAssessmentQuestions(int assessmentId) async {
    final json = await _client.get<Map<String, dynamic>>(
